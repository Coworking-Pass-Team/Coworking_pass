              ...
              <>
                ...
                {cart.map((item) => (
                  ...
                  >
                    ...
                    <div className="bg-plaster-dark/25 p-3 rounded-xl border border-soot/8 space-y-2.5">
                      <div className="flex items-center justify-between">
                        <label className="text-[11px] font-semibold text-soot flex items-center gap-1.5">
                          <Calendar size={13} className="text-moss" />
                          <span>Choose Booking Start Date</span>
                        </label>
                        <span className="text-[10px] text-moss font-medium">
                          {item.endDate && item.endDate !== item.startDate
                            ? `Ends ${formatDateNice(item.endDate)}`
                            : 'Single Day'}
                        </span>
                      </div>
                      ...
                      <input
                        ...
                        className="w-full bg-white text-soot text-xs px-3 py-2 rounded-lg border border-soot/15 focus:outline-hidden focus:ring-2 focus:ring-soot/20 font-medium cursor-pointer"
                      />

                      {/* Hourly Specific Controls */}
                      {item.plan === 'hourly' && (
                        <div className="grid grid-cols-2 gap-2 pt-1 border-t border-soot/6">
                          <div>
                            <label className="text-[10px] font-medium text-moss flex items-center gap-1 mb-1">
                              <Clock size={11} />
                              <span>Start Time</span>
                            </label>
                            <select
                              value={item.startTime || '09:00'}
                              onChange={(e) => updateCartItem(item.id, { startTime: e.target.value })}
                              className="w-full bg-white text-soot text-xs px-2.5 py-1.5 rounded-lg border border-soot/15 font-medium cursor-pointer"
                            >
                              {TIME_SLOTS.map((t) => (
                          ...
                          <div>
                            ...
                            <select
                              value={item.durationHours || 1}
                              onChange={(e) =>
                                updateCartItem(item.id, { durationHours: parseInt(e.target.value) || 1 })
                              }
                              className="w-full bg-white text-soot text-xs px-2.5 py-1.5 rounded-lg border border-soot/15 font-medium cursor-pointer"
                            >
                      ...
                      {/* Monthly Specific Controls */}
                      {item.plan === 'monthly' && (
                        <div className="pt-1 border-t border-soot/6">
                          <div className="flex items-center justify-between">
                            <label className="text-[10px] font-medium text-moss">Duration (Months)</label>
                            <select
                              value={item.durationMonths || 1}
                              onChange={(e) =>
                                updateCartItem(item.id, { durationMonths: parseInt(e.target.value) || 1 })
                              }
                              className="bg-white text-soot text-xs px-2.5 py-1 rounded-lg border border-soot/15 font-medium cursor-pointer"
                            >
                      ...
                      <div className="text-[11px] text-moss bg-white/80 px-2.5 py-1.5 rounded-lg border border-soot/8 flex items-center justify-between font-mono">
                        <span className="text-soot font-medium">
                          {formatDateNice(item.startDate)}
                          {item.endDate && item.endDate !== item.startDate ? ` → ${formatDateNice(item.endDate)}` : ''}
                        </span>
