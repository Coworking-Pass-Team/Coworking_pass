...
- **المصادقة والأمان (Auth & Security):** JWT (JSON Web Tokens)، تشفير كلمات المرور بـ bcryptjs، نظام صلاحيات مبني على الأدوار (RBAC)، والتحقق من رموز الدخول المتغيرة (TOTP)، مع دعم معيار اختيار مفتاح الدولة الدولي (Country Code Picker) ومكتبة التحقق المعيارية للهواتف (E.164 format) بدلاً من الفحص برمز تعبيري سعودي ثابت.
